...
from catchat.extensions import db, login_manager, csrf, moment, socketio, oauth
...
def register_commands(app):
    ...
    @app.cli.command()
    @click.option('--messages', default=300, help='Quantity of messages, default is 300.')
    def forge(messages):
        """Generate fake data."""
        import random
        ...
        admin = User(nickname='Jack Chen', email='admin@example.org')
        ...
        for i in range(messages):
            message = Message(
                ...
                body=fake.sentence(),
                timestamp=fake.date_time_between('-10d', '-1d')
            )
